...
struct cartmesh : flecsi::topo::specialization<flecsi::topo::narray, cartmesh> {
  ...
  using coord = flecsi::topo::narray<cartmesh>::coord;
  using hypercube = flecsi::topo::narray<cartmesh>::hypercube;
  using coloring_definition = flecsi::topo::narray<cartmesh>::coloring_definition;
  ...
  struct interface : B {
    ...
    std::size_t size() {
      if constexpr(SE == interior) {
        ...
        return B::template size<index_space::cells, A, B::range::logical>();
      }
      else if(SE == all) {
        return B::template size<index_space::cells, A, B::range::all>();
      }
      else if(SE == global) {
        return B::template size<index_space::cells, A, B::range::global>();
      }
    }
      ...
      if constexpr(SE == interior) {
        ...
        return flecsi::topo::make_ids<index_space::cells>(
          flecsi::util::iota_view<flecsi::util::id>(
            ...
            md.logical[index_space::cells][1][A] - 1));
      }
      else if constexpr(SE == logical) {
        return B::
    ...
    double value(std::size_t i) {
      return delta() *
             (B::template offset<index_space::cells, A, B::range::global>() +
               i);
    }
    ...
    bool is_boundary(std::size_t i) {
      auto const loff =
        B::template offset<index_space::cells, A, B::range::logical>();
      ...
      if(B::template is_low<A>()) {
        ...
        auto const lsize =
          B::template size<index_space::cells, A, B::range::logical>();
        return i == (lsize - loff);
      }
      else {
        return false;
  ...
  static void init_geometry(cartmesh::accessor<flecsi::rw> m,
    coloring const & c,
  ...
  static void initialize(flecsi::data::topology_slot<cartmesh> & s,
    coloring const & c,
    grect const & geometry) {
